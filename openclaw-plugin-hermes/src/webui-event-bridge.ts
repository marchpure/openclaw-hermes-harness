...
function emitAgent(
  ...
  // 同时走 broadcast 和 nodeSendToSession，分别兼容：
  // 1. 全局订阅事件流
  // 2. 目标 session 的定向推送
...
export function createWebUiEventBridge(params: AgentHarnessAttemptParams) {
  ...
  return {
    ...
    assistantDelta(deltaText: string) {
      ...
      // agent 事件仍然逐段发，chat 事件只做轻微节流，这样 WebUI 既能看到细粒度
      // 进度，又不至于因为超高频 chat delta 把前端和 gateway 压垮。
    ...
    thinkingDelta(deltaText: string) {
      ...
      // thinking 目前只作为 agent 事件暴露，不拼接进 chat message，避免把
      // reasoning 原文污染到最终 assistant 展示区。
