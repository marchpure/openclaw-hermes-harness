...
#
# v6 在 v5 基础上增量加入官方 /model 路径的 auto harness selection 配置:
...
# - 默认设置 agents.defaults.agentRuntime.id=auto。
# - 注册 hermes/default -> alias hermes，并为默认 ark 模型注册 alias ark。
# - 不清理任何历史会话里的 agentHarnessId / agentRuntimeOverride pin。
...
normalize_runtime_entries() {
    ...
    if command -v jq >/dev/null 2>&1; then
        ...
        jq \
          ...
          --arg ark_model_ref "${ARK_MODEL_REF}" \
          ...
          | (if ($ark_model_ref | length) > 0 then $ark_model_ref else (.agents.defaults.model.primary // "") end) as $ark_ref
          | .agents.defaults.models["hermes/default"] = ((.agents.defaults.models["hermes/default"] // {}) + { "alias": "hermes" })
          | if ($ark_ref | startswith("ark/")) then
              .agents.defaults.models[$ark_ref] = ((.agents.defaults.models[$ark_ref] // {}) + { "alias": "ark" })
            else . end
          | .models = (.models // {})
        ...
        tmp="$(mktemp "${config_dir}/openclaw.json.tmp.XXXXXX")"
        TEMP_DIRS+=("${tmp}")
        python3 - "${OPENCLAW_CONFIG}" "${tmp}" "${PLUGIN_CONFIG_KEY}" "${CONTAINER_NAME}" "${ACP_TCP_HOST}" "${ACP_TCP_PORT}" "${ARK_MODEL_REF}" <<'PYEOF' || die "OpenClaw 配置归一化失败: python 写入失败"
import json, sys
cf, tmp, pk, cn, tcp_host, tcp_port, ark_model_ref = sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], sys.argv[5], int(sys.argv[6]), sys.argv[7].strip()
with open(cf) as f:
    data = json.load(f)
...
hermes_model_entry = agent_models.get("hermes/default") if isinstance(agent_models.get("hermes/default"), dict) else {}
hermes_model_entry["alias"] = "hermes"
agent_models["hermes/default"] = hermes_model_entry
primary_model = defaults.get("model", {}).get("primary") if isinstance(defaults.get("model"), dict) else None
ark_ref = ark_model_ref or (primary_model if isinstance(primary_model, str) else "")
if ark_ref.startswith("ark/"):
    ark_model_entry = agent_models.get(ark_ref) if isinstance(agent_models.get(ark_ref), dict) else {}
    ark_model_entry["alias"] = "ark"
    agent_models[ark_ref] = ark_model_entry
