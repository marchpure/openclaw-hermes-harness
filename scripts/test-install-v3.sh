...
INSTALL_V3="${SCRIPT_DIR}/install-v3.sh"
UNINSTALL_V3="${SCRIPT_DIR}/uninstall-v3.sh"
...
run_low_version_case() {
    ...
    if bash "${INSTALL_V3}" >/tmp/install-v3-low-version.log 2>&1; then
        echo "[low-version] 低版本 OpenClaw 未被拒绝" >&2
        return 1
    fi
    grep -q "2026.4.15" /tmp/install-v3-low-version.log || {
...
echo "OK install-v3/uninstall-v3 regression suite passed"
