...
import type { ContextLevel, CredentialScope, WritebackLevel, StrategyTriple } from "./types.js";
...
const CREDENTIAL_SIGNALS: Array<{ pattern: RegExp; keys: string[] }> = [
  ...
  {
    pattern: /home\s*assistant|ha_|智能家居|灯|场景|自动化.*家/i,
    keys: ["HASS_TOKEN", "HASS_URL"],
  },
  ...
  {
    pattern: /通知|notify|send.*message|发.*消息/i,
    keys: ["TELEGRAM_BOT_TOKEN", "DISCORD_BOT_TOKEN"],
  },
...
function inferContextLevel(task: string): {
  level: ContextLevel;
  confidence: number;
  reason: string;
} {
  ...
  if (matchesAny(task, MEMORY_SIGNALS)) {
    return {
      level: "L2",
      confidence: 0.8,
      reason: "Task references memory, preferences, or identity",
    };
  }
  ...
  if (matchesAny(task, TOOL_SIGNALS)) {
    return {
      level: "L1",
      confidence: 0.75,
      reason: "Task requires tool execution (terminal/browser/file)",
    };
  }
...
function inferCredentialScope(task: string): {
...
function inferWritebackLevel(task: string): {
  ...
  if (matchesAny(task, FULL_WRITEBACK_SIGNALS)) {
    return {
      level: "W3",
      confidence: 0.8,
      reason: "Task creates skills, cron jobs, or config changes",
    };
  }
