...
import type { WritebackLevel, DispatchResult, MemoryUpdate, AcpSessionEvent } from "./types.js";
...
async function applyMemoryUpdate(update: MemoryUpdate, workspaceDir: string): Promise<void> {
...
async function processFullWriteback(
  ...
  const cronEvents = events.filter((e) => e.type === "tool_result" && e.text?.includes("cronjob"));
 ...
 */
function parseSkillCreationEvent(event: AcpSessionEvent): { name: string; path: string } | null {
