...
import type {
  HermesDynamicToolCallOutputContentItem,
  HermesDynamicToolCallParams,
  HermesDynamicToolCallResponse,
  HermesDynamicToolSpec,
  JsonValue,
} from "./protocol.js";

export type HermesDynamicToolBridge = {
  specs: HermesDynamicToolSpec[];
  handleToolCall(params: HermesDynamicToolCallParams): Promise<HermesDynamicToolCallResponse>;
  telemetry: {
...
export function createHermesDynamicToolBridge(params: {
  tools: AnyAgentTool[];
  signal: AbortSignal;
}): HermesDynamicToolBridge {
  const toolMap = new Map(params.tools.map((tool) => [tool.name, tool]));
  const telemetry: HermesDynamicToolBridge["telemetry"] = {
...
function collectToolTelemetry(params: {
  ...
  telemetry: HermesDynamicToolBridge["telemetry"];
  isError: boolean;
}): void {
  if (params.isError) {
    return;
  }
  if (params.toolName === "cron" && readFirstString(params.args, ["action"]) === "add") {
  ...
  if (!isMessagingTool(params.toolName) || !isMessagingToolSendAction(params.toolName, params.args)) {
    return;
  }
