import { DEFAULT_CONFIG, type HermesPluginConfig as HermesAcpPluginConfig } from "./acp/types.js";
...
export type HermesPluginConfig = {
  ...
  /**
   * Compatibility namespace for the previous `extensions/openclaw-plugin-hermes`
   * config. The installer writes these fields at the plugin config top level,
   * so top-level values still win when both are present.
   */
