  ...
  return {
    ...
    ...(skillProjection
      ? {
          skillProjection: {
            mode:
              skillProjection.mode === "strict" || skillProjection.mode === "permissive"
                ? skillProjection.mode
                : DEFAULT_CONFIG.skillProjection.mode,
            allowExecutableAssets:
              typeof skillProjection.allowExecutableAssets === "boolean"
                ? skillProjection.allowExecutableAssets
            ...
            descriptiveOnlyAllowlist: Array.isArray(skillProjection.descriptiveOnlyAllowlist)
              ? skillProjection.descriptiveOnlyAllowlist.filter(
                  (entry): entry is string => typeof entry === "string" && entry.trim().length > 0,
                )
